...
init
{
  ...
  list@File( { .regex = ".*\\.json", .directory = serviceDirectory + "/private/", .info = true } )( listResponse );
  for ( file in listResponse.result ) {
    readFile@File( { .filename = file.info.absolutePath, .format = "json" } )( readFileResponse );
