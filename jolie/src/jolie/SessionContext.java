 ...
 * @author martin
 */
public class SessionContext extends ScopableContext
{
	...
	private class SessionMessageFuture implements Future< SessionMessage >
	{
		...
		private final ScopableContext context;

		public SessionMessageFuture(ScopableContext ctx)
		{
...
//					}
	...
	private class SessionMessageNDFuture extends SessionMessageFuture
	{
		...
		public SessionMessageNDFuture( ScopableContext ctx, String[] operationNames )
		{
			super( ctx );
	...
	private final Map< String, Deque< SessionContext.SessionMessageFuture > > messageWaiters =	new HashMap<>();
	...
	private SessionContext( Process process, ScopableContext parent, jolie.State state )
	{
	...
	public SessionContext( Process process, jolie.State state, ScopableContext parent )
	{
	 ...
	 */
	public SessionContext( Interpreter interpreter, Process process )
	{
	 ...
	 */
	public SessionContext( Process process, ScopableContext parent )
	{
		super( process, parent );
		initMessageQueues();
		assert( parent != null );
	...
	@Override
	public Future< SessionMessage > requestMessage( Map< String, InputOperation > operations, ScopableContext scope )
	{
		final SessionContext.SessionMessageFuture future = new SessionContext.SessionMessageNDFuture( scope, operations.keySet().toArray( new String[0] ) );
		...
		synchronized( messageQueues ) {
			...
			if ( message == null || operation == null ) {
				...
				SessionContext.SessionMessageFuture f;
	...
	{
		final SessionContext.SessionMessageFuture future = new SessionContext.SessionMessageFuture( ctx );
		...
		synchronized( messageQueues ) {
			...
			if ( message == null
				...
				while( keepRun && !queue.isEmpty() ) {
					...
					final SessionContext.SessionMessageFuture currFuture = getMessageWaiter( otherMessage.message().operationName() );
	...
	private void addMessageWaiter( InputOperation operation, SessionContext.SessionMessageFuture future )
	{
		Deque< SessionContext.SessionMessageFuture > waitersList = messageWaiters.get( operation.id() );
		if ( waitersList == null ) {
	...
	private SessionContext.SessionMessageFuture getMessageWaiter( String operationName )
	{
		Deque< SessionContext.SessionMessageFuture > waitersList = messageWaiters.get( operationName );
	...
	public void pushMessage( SessionMessage message )
	{
		synchronized( messageQueues ) {
			...
			SessionContext.SessionMessageFuture future = getMessageWaiter( message.message().operationName() );
	 ...
	 */
	@Deprecated
	public static final SessionContext currentContext() {
		Thread t = Thread.currentThread();
		if (t instanceof JolieExecutorThread) {
