...
public class Parser
{
	...
	private Process parseChoiceProcess()
		throws IOException, ParserException
	{
		ChoiceProcess choiceProc = new ChoiceProcess();
		...
		while( !stop ) {
			...
			if ( token.type() != Scanner.TokenType.RSQUARE )
				throwException( "] expected" );
		...
		throws IOException, ParserException
	{
		...
		try {
			if ( token.type() == Scanner.TokenType.ID ) {
			...
			} else if ( token.type() == Scanner.TokenType.OUT ) {	// out( <id> )
				getToken();
				...
				proc = new NullProcess();
				getToken();
			}
			/* else if un sacco di roba -- todo */
