...
{
	/*exec@Exec( "qdbus org.kde.plasma" )( cmdStr );
...
{
	[ goToPage( request ) ] {
		exec@Exec( cmdStr + "/Previewer goToPage " + request.pageNumber )()
	}

	[ openDocument( request ) ] {
		exec@Exec( cmdStr + "/Previewer openFile " + request.documentUrl )()
	}

	[ close( request ) ] {
		nullProcess
		//exec@Exec( cmdStr + "/Previewer quit" )()
	}

	[ currentPage()( response ) {
		exec@Exec( cmdStr + "/Previewer currentPage" )( response );
		trim@StringUtils( response )( response )
	} ] { nullProcess }

	[ currentDocument()( response ) {
		exec@Exec( cmdStr + "/Previewer currentFile" )( response );
		trim@StringUtils( response )( response )
	} ] { nullProcess }
}