...
main {
    /*[ getJolieInterface( request )( response ) {
        ...
        for( o = 0, o < #outputPort.interface.operation, o++ ) {
            ...
            if ( !is_defined( outputPort.interface.operation[ o ].response.schema.("$ref") ) ) {
                ...
                if ( is_defined( outputPort.interface.operation[ o ].response.schema ) ) {
                    if ( outputPort.interface.operation[ o ].response.schema.type == "array" ) {
        ...
        for( o = 0, o < #outputPort.interface.operation, o++ ) {
            op_parameters -> outputPort.interface.operation[ o ].parameters
            if ( is_defined( op_parameters.schema.("$ref")) && op_parameters.in == "body" ) {
                split@StringUtils( op_parameters.schema.("$ref") { .regex="#/definitions/" } )( splres )
                request_type_name = splres.result[1]
            } else {
                request_type_name = outputPort.interface.operation[ o ] + "Request"
            }
            op_resp_type -> outputPort.interface.operation[ o ].response
            if ( is_defined(op_resp_type.schema.("$ref") ) ) {
                getReferenceName@OpenApiDefinition( outputPort.interface.operation[ o ].response.schema.("$ref") )( response_type_name )
            } else {
                response_type_name = outputPort.interface.operation[ o ] + "Response"
            }
        ...
        interface_file = interface_file + "interface " + outputPort.interface + "HTTP {\n";
        interface_file = interface_file + "RequestResponse:\n";
        for( o = 0, o < #outputPort.interface.operation, o++ ) {
            interface_file = interface_file + "\t" + outputPort.interface.operation[ o ]
            if ( o < (#outputPort.interface.operation - 1) ) {
