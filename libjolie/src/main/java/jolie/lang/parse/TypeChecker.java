...
public class TypeChecker implements UnitOLVisitor {
	...
	public void visit( DeepAssignStatement n ) {
		if( n.expression() instanceof VariableExpressionNode ) {
			// TODO: check whether to invalidate all variable paths in other expression cases
			typingResult.invalidate( ((VariableExpressionNode) n.expression()).variablePath() );
		}
		typingResult.invalidate( n.path() );
	}
