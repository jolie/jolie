...
public class SemanticVerifier implements UnitOLVisitor {
	...
	private final MultiMap<String, String> inputTypeNameMap = new ArrayListMultiMap<>(); // Maps type names to the input
	...
	// private TypeDefinition rootType; // the type representing the whole session
	// state
	...
	private void addRequestResponseEqualnessCheck(RequestResponseOperationDeclaration key,
		...
		List<RequestResponseOperationDeclaration> toBeEqualList = rrToBeEqual.computeIfAbsent(key,
				k -> new LinkedList<>());
	...
	/**
	 * Finds declared symbol by name, perform lookup at both from a program's source
	 * or from the given
	 * context. Since the context can be differ from program source if the symbol
	 * declared in the
	...
	public void visit(final InputPortInfo n) {
		...
		if (n.location() != null
				&& !(n.location() instanceof ConstantStringExpression
						|| n.location() instanceof VariableExpressionNode)) {
		...
		for (InputPortInfo.AggregationItemInfo item : n.aggregationList()) {
			for (String portName : item.outputPortList()) {
				...
				if (outputPort == null) {
					...
					if (item.interfaceExtender() != null) {
						outputPort.operations().forEach(opDecl -> {
							final TypeDefinition requestType = opDecl instanceof OneWayOperationDeclaration
									? ((OneWayOperationDeclaration) opDecl).requestType()
									: ((RequestResponseOperationDeclaration) opDecl).requestType();
							/*
							 * if ( requestType instanceof TypeInlineDefinition == false ) { error( n,
							 * "input port " +
							 * n.id() + " is trying to extend the type of operation " + opDecl.id() +
							 * " in output port "
							 * + outputPort.id() +
				...
				/*
				 * else { for( OperationDeclaration op : outputPort.operations() ) { if (
				 * opSet.contains( op.id() )
				 * ) { error( n, "input port " + n.id() + " declares duplicate operation " +
				 * op.id() +
				 * " from aggregated output port " + outputPort.id() ); } else { opSet.add(
				 * op.id() ); } } }
				 */
			}
		}
	...
	public void visit(CorrelationSetInfo n) {
		...
		/*
		 * VariablePathNode varPath; List< Pair< OLSyntaxNode, OLSyntaxNode > > path;
		 * for( List<
		 * VariablePathNode > list : n.variables() ) { varPath = list.get( 0 ); if (
		 * varPath.isGlobal() ) {
		 * error( list.get( 0 ), "Correlation variables can not be global" ); } path =
		 * varPath.path(); if (
		 * path.size() > 1 ) { error( varPath,
		 * "Correlation variables can not be nested paths" ); } else if
		 * ( path.get( 0 ).value() != null ) { error( varPath,
		 * "Correlation variables can not use arrays" );
		 * } else { correlationSet.add( ((ConstantStringExpression)path.get( 0
		 * ).key()).value() ); } }
		 */
	}
	...
	/*
	 * todo: Check that the output port of the forward statement is right wrt the
