 ...
 */
final class DefaultJsscChannelConfig
	extends DefaultChannelConfig
	implements JsscChannelConfig {
	...
	public Map<ChannelOption<?>, Object> getOptions() {
		return getOptions(
			super.getOptions(),
			JsscChannelOption.BAUD_RATE,
			JsscChannelOption.DTR,
			JsscChannelOption.RTS,
			JsscChannelOption.STOP_BITS,
			JsscChannelOption.DATA_BITS,
			JsscChannelOption.PARITY_BIT,
			JsscChannelOption.WAIT_TIME
		);
	}
	...
	public <T> T getOption( ChannelOption<T> option ) {
		if ( option == JsscChannelOption.BAUD_RATE ) {
			return ( T ) getBaudrate();
		}
		if ( option == JsscChannelOption.DTR ) {
			return ( T ) isDtr();
		}
		if ( option == JsscChannelOption.RTS ) {
			return ( T ) isRts();
		}
		if ( option == JsscChannelOption.STOP_BITS ) {
			return ( T ) getStopbits();
		}
		if ( option == JsscChannelOption.DATA_BITS ) {
			return ( T ) getDatabits();
		}
		if ( option == JsscChannelOption.PARITY_BIT ) {
			return ( T ) getParitybit();
		}
		if ( option == JsscChannelOption.WAIT_TIME ) {
	...
	public <T> boolean setOption( ChannelOption<T> option, T value ) {
		...
		} else if ( option == JsscChannelOption.DTR ) {
			setDtr( ( Boolean ) value );
		} else if ( option == JsscChannelOption.RTS ) {
			setRts( ( Boolean ) value );
		} else if ( option == JsscChannelOption.STOP_BITS ) {
			setStopbits( ( Integer ) value );
		} else if ( option == JsscChannelOption.DATA_BITS ) {
			setDatabits( ( Integer ) value );
		} else if ( option == JsscChannelOption.PARITY_BIT ) {
			setParitybit( ( Integer ) value );
		} else if ( option == JsscChannelOption.WAIT_TIME ) {
