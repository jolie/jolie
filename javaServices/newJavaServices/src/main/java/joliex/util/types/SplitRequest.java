 ...
 * @see #construct()
 */
public final class SplitRequest extends ImmutableStructure<JolieString> {
    
    public String regex() { return getFirstChildValue( "regex", JolieString.class ).get(); }
    public Optional<Integer> limit() { return getFirstChildValue( "limit", JolieInt.class ); }
    ...
    static <T> NestedBuilder<T> constructNested( Function<SplitRequest, T> doneFunc ) { return new NestedBuilder<>( doneFunc ); }
    static <T> NestedBuilder<T> constructNested( Function<SplitRequest, T> doneFunc, JolieValue t ) { return new NestedBuilder<>( doneFunc, t ); }
    
    static InlineListBuilder constructList() { return new InlineListBuilder(); }
    
    static <T> NestedListBuilder<T> constructNestedList( Function<List<SplitRequest>, T> doneFunc ) { return new NestedListBuilder<>( doneFunc ); }
    static <T> NestedListBuilder<T> constructNestedList( Function<List<SplitRequest>, T> doneFunc, SequencedCollection<? extends JolieValue> c ) { return new NestedListBuilder<>( doneFunc, c ); }
    
    public static InlineBuilder constructFrom( JolieValue t ) { return new InlineBuilder( t ); }
    
    public static SplitRequest createFrom( JolieValue t ) throws TypeValidationException { return constructFrom( t ).build(); }
    
    public static Value toValue( SplitRequest t ) { return JolieValue.toValue( t ); }
    public static SplitRequest fromValue( Value value ) throws TypeCheckingException { return Builder.buildFrom( value ); }
    ...
    static abstract class Builder<B> extends StructureBuilder<JolieString, B> {
        ...
        private static final Map<String,FieldManager<?>> FIELD_MAP = Map.of(
            ...
            "limit", FieldManager.createNative( 0, 1, JolieInt::fromValue, JolieInt::createFrom )
        );
        ...
        public B setLimit( JolieInt contentEntry ) { return putAs( "limit", contentEntry, JolieValue::create ); }
        public B setLimit( Integer valueEntry ) { return putAs( "limit", valueEntry, JolieValue::create ); }
        public B replaceLimit( UnaryOperator<Integer> valueOperator ) { return computeAs( "limit", (n,v) -> valueOperator.apply( v ), s -> JolieInt.class.cast( s.content() ).value(), JolieValue::create ); }
        
        protected SplitRequest validatedBuild() throws TypeValidationException {
            validateChildren( FIELD_MAP );
            
            return new SplitRequest( this );
        }
        
        private static SplitRequest buildFrom( Value value ) throws TypeCheckingException {
            InlineBuilder builder = SplitRequest.construct();
    ...
    public static class InlineBuilder extends Builder<InlineBuilder> {
        ...
        public SplitRequest build() throws TypeValidationException { return validatedBuild(); }
    }
    
    public static class NestedBuilder<T> extends Builder<NestedBuilder<T>> {
        
        private final Function<SplitRequest, T> doneFunc;
        
        private NestedBuilder( Function<SplitRequest, T> doneFunc, JolieValue t ) { super( t ); this.doneFunc = doneFunc; }
        private NestedBuilder( Function<SplitRequest, T> doneFunc ) { this.doneFunc = doneFunc; }
    ...
    static abstract class ListBuilder<B> extends StructureListBuilder<SplitRequest, B> {
        
        protected ListBuilder( SequencedCollection<? extends JolieValue> elements ) { super( elements.parallelStream().map( SplitRequest::createFrom ).toList() ); }
    ...
    public static class InlineListBuilder extends ListBuilder<InlineListBuilder> {
        ...
        public List<SplitRequest> build() { return super.build(); }
    }
    
    public static class NestedListBuilder<T> extends ListBuilder<NestedListBuilder<T>> {
        
        private final Function<List<SplitRequest>, T> doneFunc;
        
        private NestedListBuilder( Function<List<SplitRequest>, T> doneFunc, SequencedCollection<? extends JolieValue> c ) { super( c ); this.doneFunc = doneFunc; }
        private NestedListBuilder( Function<List<SplitRequest>, T> doneFunc ) { this.doneFunc = doneFunc; }
