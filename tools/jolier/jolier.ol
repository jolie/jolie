...
main {
    if ( #args < 4 || #args > 5 ) {
        println@Console("Usage: jolier <service_filename> <input_port> <router_host> <output_folder> [easy_interface true|false]")()
    ...
    getJesterConfig@Jolie2Swagger( swagger )( f.content );
    f.format = "text"
    ...
    loadEmbeddedService@Runtime( { .filepath="--trace --stackTraces -C API_ROUTER=\"socket://localhost:8000\" services/jester/router.ol", .type="Jolie"} )()
    linkIn( lock )
