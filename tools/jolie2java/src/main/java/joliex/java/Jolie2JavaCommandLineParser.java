...
public class Jolie2JavaCommandLineParser extends CommandLineParser {
	...
	public Integer translationTarget() {
		return translationTarget;
	}

	public Boolean overwriteServices() {
		return overwriteServices;
	}
	...
	private static class JolieDummyArgumentHandler implements CommandLineParser.ArgumentHandler {
		...
		public int onUnrecognizedArgument( List< String > argumentsList, int index ) throws CommandLineException {
			...
			case "--translationTarget" -> {
				index++;
				try {
					translationTarget = Integer.parseInt( argumentsList.get( index ) );
				} catch( NumberFormatException e ) {
				}
			}
			case "--overwriteServices" -> {
				index++;
				overwriteServices = Boolean.parseBoolean( argumentsList.get( index ) );
			}
			...
			case "--sourcesPackage" -> {
				index++;
				sourcesPackage = argumentsList.get( index );
			}
			...
			default -> throw new CommandLineException(
				"Unrecognized command line option: " + argumentsList.get( index ) );
			}
	...
	protected String getHelpString() {
		return new StringBuilder()
			...
			.append( "                  [ --translationTarget <0:services | 1:interfaces | 2:types> (default=0) ]" )
			.append( "\n" )
			.append( "                  [ --overwriteServices <true|false> (default=false) ]" ).append( "\n" )
			.append( "                  [ --outputDirectory <path> (default=\"" )
			.append( JavaDocumentCreator.DEFAULT_OUTPUT_DIRECTORY ).append( "\") ]" ).append( "\n" )
			.append( "                  [ --sourcesPackage <package> (default=\"" )
			.append( JavaDocumentCreator.DEFAULT_SOURCES_PACKAGE ).append( "\") ]" ).append( "\n" )
