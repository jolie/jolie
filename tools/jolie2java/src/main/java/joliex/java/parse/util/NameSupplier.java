...
public class NameSupplier implements Supplier<String> {
	...
	public NameSupplier( Supplier<String> name ) { this( name, new HashSet<>() ); }
	...
	public static NameSupplier from( String name, TypeDefinition typeDefinition ) { return new NameSupplier( () -> NameFormatter.getJavaName( name, typeDefinition ) ); }
	...
	private synchronized String cacheName() {
		...
		if ( !scope.add( cachedName ) )
            throw new InvalidNameException( "Class name would hide outer class, qualified name=\"" + NameFormatter.qualifiedName( cachedName, scope ) + "\"." );
		return cachedName;
	}
}