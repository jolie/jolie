...
public class JavaGWTDocumentCreator {
	...
	private static final HashMap< NativeType, String > JAVA_NATIVE_EQUIVALENT = new HashMap< NativeType, String >();
	private static final HashMap< NativeType, String > JAVA_NATIVE_METHOD = new HashMap< NativeType, String >();
	private static final HashMap< NativeType, String > JAVA_NATIVE_CHECKER = new HashMap< NativeType, String >();
		...
		for( Entry< String, OperationDeclaration > operationEntry : operations.entrySet() ) {
			...
			if( operation instanceof RequestResponseOperationDeclaration ) {
				...
				stringBuilder.append( "import " ).append( namespace ).append( ".types." )
					.append( requestResponseOperation.requestType().id() ).append( ";\n" );
				...
				operationCallBuilder.append( getPortOperationMethod( requestResponseOperation.id(),
					requestResponseOperation.requestType().id() ) );
	...
	private void variableCreate( StringBuilder stringBuilder, TypeDefinition type ) {
		...
		if( Utils.hasSubTypes( type ) ) {
			...
			Iterator< Entry< String, TypeDefinition > > i = supportSet.iterator();

			while( i.hasNext() ) {

				TypeDefinition subType = i.next().getValue();
